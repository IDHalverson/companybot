...
const blockMatchesPattern = (block, regExpression, allTextFoundObj) => {
  ...
  if (block.text && block.text.text && typeof block.text.text === "string") {
    allTextFoundObj.allTextFound =
      allTextFoundObj.allTextFound + block.text.text;
...
const ifConfiguredPassAlongMessage = async ({ message, payload, context }) => {
  ...
  try {
    ...
    if (
      !process.env.EXECUTE_PASSALONG_FOR_CHANNELS.includes(
        `<${payload.channel}>`
      )
    )
      return;
    ...
    await asyncForEach(configurations, async (configuration) => {
      ...
      const senderConfigs = senderIdConfig.split("{")[1]
        ? JSON.parse(senderConfigsAsTxt)
        : {};

      const matchesChannel =
        !originationChannel || originationChannel === payload.channel;
      if (matchesChannel) {
        const matchesUser = !senderId || message.user === senderId;
        const matchesBot =
          !senderId ||
          (message.bot_profile && message.bot_profile.app_id === senderId) ||
          (message.bot_id && message.bot_id === senderId);
        if (matchesUser || matchesBot) {
          ...
          const matchFoundInAttachments =
            message.attachments &&
            message.attachments[0] &&
            ((message.attachments[0].blocks &&
              message.attachments[0].blocks.some((block) =>
                blockMatchesPattern(block, regExpression, allTextFoundObj)
              )) ||
              (message.attachments[0].title &&
                message.attachments[0].title.match(regExpression)) ||
              (message.attachments[0].fallback &&
                message.attachments[0].fallback.match(regExpression)));
          const matchFoundInBlocks =
            message.blocks &&
          ...
          if (
            matchFoundInText ||
            matchFoundInAttachments ||
            matchFoundInBlocks
          ) {
            const customTransform = Transformers[transformerName];
            const { username, icon_emoji, text, blocks, attachments } =
              (customTransform &&
                (await customTransform(
                  message,
                  allTextFoundObj.allTextFound,
                  axios
                ))) ||
            ...
            if (
              ...
              let mappedBlocks = (
                message.blocks
                  ? message.blocks
                  : (message.attachments[0] && message.attachments[0].blocks) ||
                    []
              )
                .filter(
                  (b) => !["rich_text", "rich_text_section"].includes(b.type)
                )
                .map((b) => ({ ...b, block_id: undefined }));
              if (!mappedBlocks.length) mappedBlocks = null;
              finalContent = getFallbackContent(mappedBlocks, {
                message,
                payload,
              });
            }
            ...
            sendTo &&
              app.client.chat.postMessage({
                ...
                username: username
                  ? username
                  : `${senderConfigs.displayName || user.real_name} (clone)`,
                text: " ",
                ...finalContent,
              });
          }
